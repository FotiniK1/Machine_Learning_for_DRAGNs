...
for i in range(100):
    file_name = f"seed_{i}/predictions.csv"
    #bin_edges_name = f"../kNN_Classification/seed_{seed_list[i]}/bin_edges.csv"
    #bin_median_name = f"../kNN_Classification/seed_{seed_list[i]}/bin_median.csv"
    bin_edges_name = f"../../kNN_RF/kNN_Classification/seed_{seed_list[i]}/bin_edges.csv"
    bin_median_name = f"../../kNN_RF/kNN_Classification/seed_{seed_list[i]}/bin_median.csv"
    try:
        ...
        z_spec = np.array(predictions[:,0])
        z_photo = np.array(predictions[:,1])
    ...
    #except:
     #   print("Seed: " + str(seed_list[i]) + " (" + str(i) + ") missing predictions.")
        ...
        print(f"Seed: {seed_list[i]} ({i}) missing predictions or encountered an error. Error: {e}")
