...
def main():
    ...
    dataset = "../../../Data/below70_dataset.fits"

    des_cols = ["zphot", "gmag", "rmag", "imag", "zmag", "e_gmag", "e_rmag", "e_imag", "e_zmag",
        "W1Mag", "W2Mag", "W3Mag", "W4Mag","e_W1Mag", "e_W2Mag", "e_W3Mag", "e_W4Mag"]

    data = read_fits(dataset, des_cols)

    data[np.where(data[:,-4] == -99), -4] = np.nanmax(data[:,-4])
    data[np.where(data[:,-3] == -99), -3] = np.nanmax(data[:,-3])
    data[np.where(data[:,-2] == -99), -2] = np.nanmax(data[:,-2])
    data[np.where(data[:,-1] == -99), -1] = np.max(data[:,-1])

    data[np.where(np.isnan(data[:,-4])), -4] = np.nanmax(data[:,-4])
    data[np.where(np.isnan(data[:,-3])), -3] = np.nanmax(data[:,-3])
    data[np.where(np.isnan(data[:,-2])), -2] = np.nanmax(data[:,-2])
    data[np.where(np.isnan(data[:,-1])), -1] = np.nanmax(data[:,-1])
    ...
    for component in np.arange(gmm_model.n_components):
        ...
        if not os.path.isfile(test_file_name):
            ...
            comp_y_train = y_vals_train_bin[np.where(clusters_train == component)]
            comp_x_train = x_vals_train[np.where(clusters_train == component)]
            comp_y_test = y_vals_test_bin[np.where(clusters_test == component)]
    ...
    for component in np.arange(1,gmm_model.n_components):
        try:
            ...
            print(f"No sources predicted for component {component}")


    test_table_stack["z_spec"], test_table_stack["prediction"], _, _ = \
        bin_data_func(test_table_stack["z_spec"], test_table_stack["prediction"], num_classification_bins, bin_edges=bin_edge, bin_medians=bin_median)
