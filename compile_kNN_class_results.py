...
#working_directory = "/Users/43302335/Desktop/all/MRes/Year_2/KL_Scripts_&_Data/GitHub/Scripts/kNN_Classification"
...
for i in range(len(seed_list)):
    file_name = "seed_" + str(seed_list[i]) + "/predictions.csv"

#for i in seed_list: #range(100):
    #file_name = "seed_" + str(i) + "/predictions.csv"
    #file_name = os.path.join(working_directory, f"seed_{i}/predictions.csv")
    try:
        ...
        out_sigma.append(outlier_rate(norm_residual(np.array(predictions[:,0]), np.array(predictions[:,1])), (2*sigma[i])))
        acc.append(accuracy_score(np.array(predictions[:,0].astype(str)), np.array(predictions[:,1].astype(str)), normalize=True))
