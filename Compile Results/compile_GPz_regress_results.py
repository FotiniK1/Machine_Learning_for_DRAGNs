...
for i in range(100):
    ...
    file_name = f"seed_{i}/predictions_seed_{seed_list[i]}_comp_30.csv"
    predictions = pd.read_csv(file_name) 
    out_rate.append(outlier_rate(norm_residual(np.array(predictions["z_spec"]), np.array(predictions["prediction"]))))
    sigma.append(norm_std_dev(norm_residual(np.array(predictions["z_spec"]), np.array(predictions["prediction"]))))
    nmad.append(norm_mad((np.array(predictions["z_spec"]) - np.array(predictions["prediction"]))))
    mse.append(mean_squared_error(np.array(predictions["z_spec"]), np.array(predictions["prediction"])))
    
    out_sigma.append(outlier_rate(norm_residual(np.array(predictions["z_spec"]), np.array(predictions["prediction"])), (2*sigma[i])))
...
print("MSE    Std Error")
print(np.mean(mse), (np.std(mse) / np.sqrt(len(mse))))
